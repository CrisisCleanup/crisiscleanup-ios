...
final class NetworkAuthTests: XCTestCase {
    func testAuthSuccessResult() throws {
        let actual = Bundle(for: NetworkAuthTests.self)
            .loadJson("authResponseSuccess", NetworkAuthResult.self)

        XCTAssertNil(actual.errors)
        ...
        XCTAssertEqual(actual.organizations!, expectedOrganization)
    }
    ...
    func testAuthFailResult() throws {
        ...
        XCTAssertNil(actual.claims)
        XCTAssertNil(actual.organizations)

        XCTAssertEqual(1, actual.errors?.count)
        let firstError = actual.errors!.first!
        XCTAssertEqual(
